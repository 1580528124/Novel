...
const modules: HoloModule[] = [
  ...
  },
  {
    ...
    mark: "SS",
    lines: ["任务信标：高", "现场通讯：可回放", "行动路线：待展开"]
  },
  {
    id: "kings",
    loreId: "dragon_kings",
  ...
  },
  {
    ...
    mark: "VII",
    lines: ["言灵目录：索引", "七宗罪：武器剖面", "龙文释读：限制访"]
  },
  {
    id: "academy",
    loreId: "academy",
  ...
  },
  {
    ...
    mark: "RAD",
    lines: ["全球异常：扫描中", "通讯截获：低", "响应链路：执行部"]
  },
  {
    id: "evidence",
    loreId: "emotion_library",
    label: "证据",
    eyebrow: "FIELD EVIDENCE LIBRARY",
    title: "现场证据/ 信号归档",
    subtitle: "执行部任务证据、实时预警信号与 NORMA 现场判断记录",
    position: [1.5, -1.08, 2.25],
    color: "#c9a96e",
    camera: [1.1, -0.42, 4.85],
    target: [1.5, -0.92, 2.25],
    mark: "TXT",
    lines: ["实时证据：每日同", "任务证据：已接入", "原文档案：不在此"]
  }
];
...
type FieldEvidenceSource = "USGS" | "NASA_EONET" | "GDACS" | "EXECUTIVE" | "NORMA" | "FALLBACK";
type FieldEvidenceStatus = "可读" | "摘要开" | "封存" | "待复";
type FieldEvidenceKind = "realtime" | "operation";
type FieldEvidenceView = "list" | "chain";
...
type FieldEvidence = {
  ...
  summary: string;
  normaJudgement: string;
  signals: string[];
  linkedOperationId?: string;
  linkedAnomalyCode?: string;
};

type FieldEvidenceFilter = "all" | "realtime" | "operation" | "restricted";
...
const disasterForecastSignals: SurveillanceAnomaly[] = [
  {
    ...
    probability: 68,
    indicators: ["浅源震群扩散", "地下空洞回声增强", "山脉带重力异"],
    clearance: 2,
    lat: 36.2048,
    lon: 138.2529,
    tone: "#d8bd66",
    summary: "NORMA 将过72 小时内的浅源地震、地壳应力、地下空洞回声进行合并计算，结果显示异常更接近大地与山之王的复苏前兆，而非普通板块释放",
    ...
    recommendation: "维持卫星与声呐链路，等待第二组深海数据"   },
  {
    ...
    probability: 74,
    indicators: ["高空电荷密度异常", "风暴路径逆转", "航空无线电噪"],
    clearance: 3,
    lat: 23.4162,
    lon: 25.6628,
    tone: "#c95e4c",
...
function localizeForecastText(text: string): string {
  ...
  return text
    ...
    .replace(/\bPoland\b/g, "波兰")
    .replace(/\bMexico\b/g, "墨西")
    .replace(/\bPhilippines\b/g, "菲律")
    .replace(/\bChina\b/g, "中国")
    .replace(/\bTaiwan\b/g, "台湾")
    .replace(/\bCalifornia\b/g, "加利福尼")
    .replace(/\bUtah\b/g, "犹他")
    .replace(/\bSouth Dakota\b/g, "南达科他")
    .replace(/\bNew Mexico\b/g, "新墨西哥")
    .replace(/\bMillard\b/g, "米勒")
    .replace(/\bJackson\b/g, "杰克")
    .replace(/\bKern\b/g, "克恩")
    .replace(/\bColfax\b/g, "科尔法克")
    .replace(/\bWidemouth 2\b/g, "宽口二号")
    .replace(/\bPotato Creek\b/g, "土豆")
...
function localizeEvidenceText(text: string) {
  return localizeForecastText(text)
    .replace(/Green notification for tropical cyclone\s+([A-Z0-9-]+)\.?\s*Population affected by Category\s*(\d+)?[^。]*/gi, (_match, name: string, category: string) => {
      return `热带气旋 ${name} 绿色警报${category ? `，${category} 级影响人口已记录` : "，受影响人口已记录"}`;
    })
    .replace(/Green flood alert in\s+([^"]+?)(?=$|[\u3002.])/gi, (_match, place: string) => {
      return `${localizeForecastText(place.trim())} 洪水绿色警报`;
    })
    .replace(/Orange flood alert in\s+([^"]+?)(?=$|[\u3002.])/gi, (_match, place: string) => {
      return `${localizeForecastText(place.trim())} 洪水橙色警报`;
    })
    .replace(/Red flood alert in\s+([^"]+?)(?=$|[\u3002.])/gi, (_match, place: string) => {
      return `${localizeForecastText(place.trim())} 洪水红色警报`;
    })
    .replace(/Green volcano alert for\s+([^"]+?)(?=$|[\u3002.])/gi, (_match, place: string) => {
      return `${localizeForecastText(place.trim())} 火山绿色警报`;
    })
    .replace(/tropical cyclone/gi, "热带气旋")
    .replace(/flood alert/gi, "洪水警报")
    .replace(/volcano alert/gi, "火山警报")
    .replace(/Population affected by Category\s*(\d+)?[^。]*/gi, (_match, category: string) => {
      return category ? `第 ${category} 级影响人口已记录` : "受影响人口已记录";
    })
    .replace(/\bGreen\b/g, "绿色")
    .replace(/\bOrange\b/g, "橙色")
    .replace(/\bRed\b/g, "红色")
    .replace(/\bnotification\b/gi, "通报")
    .replace(/\bin\b/gi, "位于")
    .replace(/\s+/g, " ")
    .trim();
}
...
const missionEvidencePacketIds = new Set((evidencePacketsData as Array<{ id: string }>).map((packet) => packet.id));

const operationFieldEvidence = ([
  {
    id: "evpkt-kuimen-sonar",
    source: "EXECUTIVE",
    kind: "operation",
    title: "水下声呐记录",
    category: "声呐",
    occurredAt: "夔门计划 / 水下接触阶段",
    locationName: "长江夔门水域",
    coordinate: "31.0000N / 109.0000E",
    king: "青铜与火之王",
    probability: 72,
    clearance: 1,
    status: "可读",
    summary: "摩尼亚赫号在目标水域捕获非自然回声。回声边界呈建筑结构特征，并在通讯失真前出现短暂增强",
    normaJudgement: "声呐形态不符合普通沉积物或自然洞穴，建议作为青铜城接触证据保留",
    signals: ["非自然回", "建筑边界", "通讯失真前增"],
    linkedOperationId: "op-kuimen-review"
  },
  {
    id: "evpkt-kuimen-comms",
    source: "EXECUTIVE",
    kind: "operation",
    title: "叶胜 / 亚纪通讯片段",
    category: "通讯",
    occurredAt: "夔门计划 / 青铜城进入阶",
    locationName: "长江夔门水下结构",
    coordinate: "31.0000N / 109.0000E",
    king: "青铜与火之王",
    probability: 76,
    clearance: 1,
    status: "可读",
    summary: "潜水组通讯在进入青铜结构后出现间歇噪声，最后有效片段包含对龙文与内部空间的确认",
    normaJudgement: "通讯衰减与声呐异常同步，应归入青铜城接触证据链",
    signals: ["通讯中断", "龙文确认", "内部空间确认"],
    linkedOperationId: "op-kuimen-review"
  },
  {
    id: "evpkt-kuimen-vitals",
    source: "EXECUTIVE",
    kind: "operation",
    title: "氧气与生命体征读",
    category: "生命体征",
    occurredAt: "夔门计划 / 撤离阶段",
    locationName: "青铜城外缘水",
    coordinate: "31.0000N / 109.0000E",
    king: "青铜与火之王",
    probability: 61,
    clearance: 1,
    status: "可读",
    summary: "氧气窗口持续缩短，生命体征在撤离阶段出现异常波动。回收建议被多次标注为高优先级",
    normaJudgement: "该记录证明任务已从侦察转入生命优先处置，不单独判定王座风险",
    signals: ["氧气窗口缩短", "生命体征波动", "回收优先级上"],
    linkedOperationId: "op-kuimen-review"
  },
  {
    id: "evpkt-kuimen-bronze-city",
    source: "EXECUTIVE",
    kind: "operation",
    title: "青铜城结构扫",
    category: "扫描",
    occurredAt: "夔门计划 / 遗迹确认阶段",
    locationName: "青铜城内部结",
    coordinate: "31.0000N / 109.0000E",
    king: "青铜与火之王",
    probability: 89,
    clearance: 2,
    status: "摘要开",
    summary: "扫描记录显示目标并非自然洞穴。结构边界、龙文残留和热源波动共同指向青铜城内部系统响应",
    normaJudgement: "遗迹存在主动响应迹象，建议关KING-01 初级档案",
    signals: ["龙文残留", "热源波动", "遗迹主动响应"],
    linkedOperationId: "op-kuimen-review"
  },
  {
    id: "evpkt-kuimen-fire-control",
    source: "EXECUTIVE",
    kind: "operation",
    title: "摩尼亚赫号火控记",
    category: "火控",
    occurredAt: "夔门计划 / 水面追击阶段",
    locationName: "摩尼亚赫号作战甲",
    coordinate: "31.0000N / 109.0000E",
    king: "青铜与火之王",
    probability: 81,
    clearance: 2,
    status: "摘要开",
    summary: "摩尼亚赫号火控链在事件后段进入待命，授权窗口短暂且不稳定。记录建议复盘火控介入时机",
    normaJudgement: "王座级风险出现时，火控授权链必须提前进入待命",
    signals: ["火控待命", "授权窗口短暂", "水面追击"],
    linkedOperationId: "op-kuimen-review"
  }
] satisfies FieldEvidence[]).filter((item) => missionEvidencePacketIds.has(item.id));
...
function getEvidenceSourceLabel(source: FieldEvidenceSource) {
  if (source === "NASA_EONET") return "NASA EONET";
  if (source === "EXECUTIVE") return "执行";
  if (source === "FALLBACK") return "本地预案";
  return source;
}

function getEvidenceFilterLabel(filter: FieldEvidenceFilter) {
  if (filter === "realtime") return "实时接入";
  if (filter === "operation") return "执行部证";
  if (filter === "restricted") return "权限受限";
  return "全部";
}

function getEvidenceSourceStatus(evidenceList: FieldEvidence[]) {
  const sources: FieldEvidenceSource[] = ["USGS", "NASA_EONET", "GDACS", "EXECUTIVE"];
  return sources.map((source) => ({
    source,
    count: evidenceList.filter((item) => item.source === source).length
  }));
}

function getEvidenceStatus(evidence: FieldEvidence, clearance: number): FieldEvidenceStatus {
  if (clearance < evidence.clearance) return "封存";
  return evidence.status;
}

function getFieldEvidenceKey(evidence: FieldEvidence) {
  return `${evidence.kind}:${evidence.id}:${evidence.coordinate}:${evidence.title}`;
}

function mapForecastSignalToEvidence(signal: SurveillanceAnomaly): FieldEvidence {
  const title = localizeEvidenceText(signal.title);
  const locationName = localizeEvidenceText(signal.location);
  const summary = localizeEvidenceText(signal.summary);
  const normaJudgement = localizeEvidenceText(signal.norma);
  ...
  return {
    ...
    kind: "realtime",
    title,
    category: signal.disasterType ?? signal.level,
    occurredAt: signal.observedAt ?? "时间待确",
    locationName,
    coordinate: signal.coordinate,
    king: signal.predictedKing ?? "未判",
    probability: signal.probability ?? 0,
    clearance: signal.clearance,
    status: signal.clearance > 2 ? "摘要开" : "可读",
    summary,
    normaJudgement,
    signals: (signal.indicators?.length ? signal.indicators : [signal.signal]).map(localizeEvidenceText),
    linkedAnomalyCode: signal.code
  };
}

function loadCachedForecastSignals(): SurveillanceAnomaly[] {
  if (typeof window === "undefined") return fallbackNormaForecastSignals;

  try {
    const cachedRaw = window.localStorage.getItem(normaForecastCacheKey);
    if (!cachedRaw) return fallbackNormaForecastSignals;
    const cached = JSON.parse(cachedRaw) as CachedNormaForecast;
    return cached.forecast?.signals?.length ? cached.forecast.signals : fallbackNormaForecastSignals;
  } catch {
    return fallbackNormaForecastSignals;
  }
}
...
const archiveBlueprints: Partial<Record<HoloModuleId, ArchiveBlueprint>> = {
  identity: {
    ...
    records: [
      { title: "路明", level: "S", status: "权限已开", detail: "学院名单、数据库访问、账户与行程由诺玛自动签发" },
      { title: "楚子", level: "A", status: "执行部王", detail: "高危任务执行人，存在信号源追踪与特殊监控记录" },
      { title: "恺撒", level: "A", status: "学生", detail: "加图索家族继承人，学生会系统权限与行动资源较高" },
      { title: "陈墨", level: "A", status: "招生 / 侧写", detail: "可作为人物观察、关系触发和任务引导型档案" },
      { title: "芬格", level: "未知", status: "EVA 隐藏接口", detail: "输入该姓名时终端切入 EVA 模式，显示异常人格界面" },
      { title: "夏弥", level: "异常", status: "权限污染", detail: "存在高级权限取得与冰窖访问相关风险记录" }
    ],
    workflow: ["读取身份签名", "比对血统等", "开启数据库访问", "检查异常授"]
  },
  missions: {
    summary: "执行部战情室只记录行动代号、风险等级、现场状态和诺玛复盘。所有任务以作战台形式展开，禁止写成普通事件介绍",
    records: [
      { title: "夔门计划", level: "MISSION-S", status: "已封/ 伤亡确认", detail: "长江水下侦察行动，目标为确认青铜城、获取王座物证并评估诺顿复苏风险", deepView: "mission_kuimen" },
      { title: "北京尼伯龙根事件", level: "MISSION-SS", status: "城市级异", detail: "北京地铁异常、隐藏站点、地下巢穴和 KING-03 双生王座确认链", deepView: "mission_beijing_nibelung" },
      { title: "日本收容", level: "MISSION-SS", status: "白王遗产 / 封存", detail: "蛇岐八家、猛鬼众、圣骸、红井和白王遗产复苏风险的地区收容链", deepView: "mission_japan_containment" },
      { title: "格陵兰冰海记", level: "MISSION-A", status: "历史事故", detail: "冰海失联、潜水服收容与执行部纪律禁令的封存来源", deepView: "mission_greenland_ice" },
      { title: "卡塞尔学院入侵事", level: "MISSION-S", status: "校内战情", detail: "学院安保层级、中央控制室警报、入侵路线和学生战斗记录", deepView: "mission_cassell_invasion" },
      { title: "青铜计划 / 二次作战", level: "MISSION-SS", status: "鱼雷处置", detail: "摩尼亚赫号二次作战、风暴鱼雷、船体损伤和龙王处决窗口", deepView: "mission_bronze_second" }
    ],
    workflow: ["任务队列", "风险等级", "指挥", "现场态势", "诺玛复盘"]
  },
  kings: {
    ...
    records: [
      ...
      { title: "大地与山之王", level: "KING-03", status: "耶梦加得 / 芬里", detail: "双生王座已确认。档案聚焦北京地铁异常、尼伯龙根入口、夏弥伪装、芬里厄巢穴与地质级灾害", deepView: "earth_mountain" },
  ...
  alchemy: {
    ...
    records: [
      { title: "七宗", level: "S", status: "武器剖面", detail: "炼金刀剑组，是青铜与火之王相关档案的关键节点" },
      { title: "言· 君焰", level: "高危", status: "可索", detail: "适合做成音节点亮与火焰波纹的交互" },
      { title: "言· ", level: "侦测", status: "可索", detail: "可连接监控预警区，表现为信息流和感知网络" },
      { title: "龙文释读", level: "封存", status: "解析", detail: "用于生成符文阵、残缺译文和权限不足反馈" },
      { title: "青铜器物", level: "A", status: "可展开", detail: "与青铜城、机关结构、炼金术历史绑定" }
    ],
    workflow: ["识别言", "读取龙文", "展开炼金", "映射使用", "标注风险"]
  },
  academy: {
    summary: "这一区域呈现卡塞尔学院的双层结构：表面大学，真实军事机构",
    records: [
      { title: "中央控制", level: "核心", status: "在线", detail: "诺玛汇聚全球学院相关信息，人类情报员进行判断" },
      { title: "冰窖", level: "封存", status: "限制访问", detail: "高危档案与异常权限事件的重要地点" },
      { title: "图书", level: "学院", status: "开", detail: "学习、档案、讨论和学院日常的入口" },
      { title: "学生", level: "组织", status: "可查", detail: "恺撒相关组织线，连接学院权力结构" },
      { title: "狮心", level: "组织", status: "可查", detail: "楚子航相关组织线，连接学院传统与战斗精神" },
      { title: "教授", level: "管理", status: "可查", detail: "昂热、施耐德、曼施坦因等人物关系入口" }
    ],
  ...
  surveillance: {
    ...
    records: [
      ...
      { title: "异常坐标定位", level: "A", status: "扫描", detail: "把龙类活动、任务地点和通讯来源转成空间坐标" },
      { title: "通讯截获", level: "A", status: "低频", detail: "可作为现场语音、邮件、警报的证据入口" },
      { title: "警报等级", level: "S", status: "自动升级", detail: "与执行部任务区联动，触发红色战情层" },
      { title: "中央控制室响", level: "核心", status: "待决", detail: "信息汇入诺玛后由学院人员分析并下发行动" }
    ],
    workflow: ["接收信号", "定位异常", "生成警报", "通知执行", "记录证据"]
  },
  evidence: {
    summary: "这是隐藏档案馆，所有模块都必须能回溯到原文证据",
    records: [
      { title: "龙族Ⅰ：火之晨曦", level: "BOOK 1", status: "已接", detail: "带有章节和情绪标注，是当前结构最完整的基础库" },
      { title: "龙族Ⅱ：悼亡者之", level: "BOOK 2", status: "已接", detail: "已按档案片段切分，后续可补章节标题" },
      { title: "龙族Ⅲ：黑月之潮", level: "BOOK 3", status: "已接", detail: "已按档案片段切分，执行部和日本任务证据量显著增加" },
      { title: "模块证据索引", level: "SYS", status: "可检", detail: "每条记录保存所属模块、关键词、置信度和原文摘录" },
      { title: "情绪样本", level: "HIDDEN", status: "隐藏", detail: "不在主界面自动出现，只作为档案馆内部资料" }
    ],
    workflow: ["筛选书", "筛选模", "搜索关键", "打开证据", "回链对应区域"]
  }
};
...
const bronzeFireSections: BronzeFireSection[] = [
    ...
    directive: "维持 KING-01 最高危标记；任何诺顿身份线索必须同步执行部与炼金资料库"   },
  {
    ...
    related: ["康斯坦丁", "诺顿", "双生", "王座共鸣", "召回反应"],
    risk: "单点清除无法确保王座终止；残留记忆或遗骸可能成为下一次复苏入口",
    directive: "涉及康斯坦丁的记录不得低KING-01 处理等级；禁止将其归入普通龙类个体"   },
  {
    id: "city",
    label: "青铜",
    level: "SITE-S",
    ...
    directive: "后续界面建议以剖面方式呈现：水域层、金属异常层、机关通路层、王座核心层"   },
  {
    ...
    label: "夔门计划",
    level: "MISSION-S",
    status: "曼斯小组 / 长江水下 / 行动简",
    fileNo: "ED-KUIMEN-ACTION",
    classification: "任务报告 / 执行部行",
    ...
    report: [
      ...
      "撤离阶段：叶胜通过蛇获得青铜城三维地图，钥匙指出向下脱出路径。计算显示他们可以脱出青铜城，但氧气不足以支撑两人同时上浮。叶胜将氧气留给亚纪，并把重要铜匣子交由其带回",
    ...
    related: ["曼斯教授", "叶胜", "酒德亚纪", "塞尔", "钥匙", "摩尼亚赫", "长江水域", "青铜"],
    ...
    directive: "建议做成扫描层：蛇形信号沿青铜城剖面游走，依次点亮门、通路、宫殿和异常反应"   },
  {
    ...
    label: "苏醒链路",
    level: "WATCH",
    status: "沉眠确认 / 宫殿接触 / 王座激",
    fileNo: "DK-KING01-WATCH",
    classification: "监控报告 / 复苏阈",
    summary: "青铜与火之王的恐怖感来自系统逐步证明其存在：龙墓搜索、水下异常、宫殿确认、王座苏醒",
    report: [
      "建议将链路拆SEARCHING、CONTACT、PALACE CONFIRMED、AWAKENING、KING ACTIVE 五个状态",
      "每进入一个状态，界面警报应同步升级：青铜环从冷光转为火红，任务等级由 S 上调SS"     ],
    related: ["龙墓搜索", "水下异常", "宫殿确认", "复苏阈", "执行部响"],
    risk: "一旦进KING ACTIVE，档案不再适用检索模式，必须转入战斗决策模式",
    ...
    directive: "建议在青铜城剖面外增加历史遮罩层，点击后从人类地图切换到龙族真实坐标"   },
  {
    ...
    related: ["龙文", "青铜", "铸城记录", "炼金", "权限解析"],
    risk: "错误释读可能触发机关、误开门禁或唤醒沉眠结构",
    directive: "建议ALCHEMY VAULT 联动，制作逐字解析、残缺译文和权限不足反馈"   },
  {
    id: "casualty",
    label: "牺牲档案",
    level: "CASUALTY",
    status: "叶胜 / 酒德亚纪 / 塞尔/ 现场判定",
    ...
    related: ["叶胜", "酒德亚纪", "塞尔", "曼斯教授", "摩尼亚赫", "黄铜", "青铜", "言灵·蛇"],
    ...
    directive: "界面建议做成黑匣子报告：左侧列姓名、身份、任务职责、最终状态；右侧按时间轴展示氧气耗尽、物证上浮、失联、遗体发现四个关键节点"   },
  {
    ...
    label: "复苏物证",
    level: "RELIC",
    status: "黄铜/ 骨殖/ 匣子 / 龙骨十字",
    fileNo: "AL-RELIC-NORTON",
    classification: "物证报告 / 复苏载体",
    ...
    related: ["黄铜", "骨殖", "诺顿之卵", "金属", "七宗", "龙骨十字", "叶胜", "酒德亚纪"],
    risk: "物证链同时具备研究价值和灾难价值：它既能证明龙王存在，也可能成为下一次复苏、武器失控或学院内部争夺的触发点",
    directive: "界面建议做成封存柜：每件物证显示编号、取得者、取得地点、当前状态和关联风险；点击物证时不要给长段介绍，而是展开‘收容记/ 异常反应 / 关联任务’"   },
  {
    id: "bronze_plan",
    label: "青铜计划 / 二次作战",
    level: "MISSION-SS",
    status: "曼施坦因指挥 / 水下 A-B / 风暴鱼雷",
    ...
    report: [
      "行动时间010 2 13 日夜，春节前夜。行动平台仍为摩尼亚赫号，位置位于三峡水库相关水域，目标区域为地震后重新暴露的青铜城",
    ...
    related: ["青铜计划", "曼施坦因", "恺撒", "", "诺诺", "路明", "风暴鱼雷", "摩尼亚赫", "青铜"],
    ...
    directive: "界面建议做成正式作战面板：上方显示行动时间、指挥链、编组和武器状态；中段用时间轴呈现‘进入青铜城 / 放置炸弹 / 通讯中断 / 船体破损 / 近距诱导 / 鱼雷发射 / 目标命中’"   },
  {
    ...
    related: ["七宗", "炼金武器", "屠龙", "权限封锁", "武器剖面"],
    risk: "武器信息过早解封会削弱禁忌感，也会使炼金区失去后续展开空间",
    directive: "点击该栏目后续应跳转ALCHEMY VAULT 的武器剖面层"   }
];
...
const skyWindSections: BronzeFireSection[] = [
  {
    ...
    related: ["天空与风之王", "大地与山之王", "北京尼伯龙根", "镰鼬", "恺撒", "王恭"],
    ...
    directive: "界面标注 KING-02 / WIND THRONE，并在元信息中显示：王座本体未确认现身，风系样本已收录"   },
  {
    ...
    report: [
      ...
      "记录限制：当前页面应使用“未解封”“待判定”“王座空位”等档案措辞，不填入未经学院确认的姓名；这样能维KING-02 的封存等级"     ],
    related: ["双生王座", "天空与风之王", "主副节点", "复苏钥匙", "王座共鸣", "未解封档"],
    ...
    directive: "在视觉上保留一个空置副王座槽位：主投影显示风域，副节点以锁定环、缺失编号和未解封警示呈现"   },
  {
    ...
    related: ["风元", "气压", "声音传播", "真空切割", "高速目", "群体灾害"],
    ...
    directive: "归档时优先记录等压线、声波回环与透明风刃痕迹；未出现连续王座信号前，不启动完整行动时间轴"   },
  {
    ...
    related: ["镰鼬", "镰鼬女皇", "吸血", "言灵·镰", "贤者之", "尼伯龙根"],
    ...
    directive: "界面中可显示标本柜：SPECIMEN-A 活体镰鼬、SPECIMEN-B 女皇、SPECIMEN-C 吸血镰"   },
  {
    ...
    related: ["言灵·镰", "阴流", "风王之瞳", "恺撒", "矢吹", "风域"],
    risk: "风系言灵极易被误并为同一类风域能力。档案必须区分侦测型、切割型、风暴型与王座级权柄，禁止以低阶言灵代KING-02 本体判定",
    directive: "建立风系言灵分级索引：记录序列号、释放者、作用范围、攻击性、失控阈值与王座级相似度；相似度不足 80% 的记录不得上调至 KING-02 预警"   },
  {
    ...
    report: [
      "指标一：尼伯龙根风域裂缝。现实地点出现风系生物外泄、声音定位异常或历史灾害重合时，应提KING-02 监测等级",
    ...
    related: ["尼伯龙根裂缝", "王恭", "镰鼬", "声学异常", "贤者之", "风域外泄"],
    ...
    directive: "监测矩阵维持四项并列：裂缝、群体数量、声学污染、诱饵反应。任一指标异常扩大时，自动上调 KING-02 观察等级"   },
  {
    ...
    related: ["封锁", "诱饵", "远程炼金武器", "高速撤", "保密协议", "四大君主级作"],
...
const earthMountainSections: BronzeFireSection[] = [
  {
    ...
    related: ["大地与山之王", "耶梦加得", "芬里", "北京地铁", "尼伯龙根", "地动异常"],
    ...
    directive: "界面应把两个节点并列呈现：夏弥节点显示身份伪装和关系链，芬里厄节点显示地下巢穴和灾害半径"   },
  {
    ...
    related: ["北京地铁", "隐藏站点", "地下隧道", "地铁列车", "尼伯龙根", "王恭"],
    ...
    directive: "建立三层坐标档案：地表城市、地下交通、尼伯龙根巢穴。三层坐标必须同步校准，禁止单独引用地铁图作为最终坐标"   },
  {
    ...
    summary: "夏弥是耶梦加得进入人类关系网的身份，也KING-03 档案中最容易造成执行判断迟疑的情感层",
    report: [
    ...
    related: ["夏弥", "耶梦加得", "楚子", "路明", "学院日常", "身份暴露"],
    risk: "人类身份会削弱执行人员对龙王本体的即时判断，并在任务结束后留下长期精神残留",
    directive: "该条目应与人物档案联动，但在龙王档案内保KING-03 级别标记，避免被降级为普通角色资料"   },
  {
    id: "fenrir",
    label: "芬里厄巢",
    level: "NEST",
    status: "地下深处 / 守巢节点 / 龙王级体",
    ...
    related: ["芬里", "地下巢穴", "姐姐", "龙王级目", "大地权柄", "守巢"],
    ...
    directive: "视觉上建议显示被城市剖面压住的巨大巢穴轮廓，芬里厄节点置于地铁线下方"   },
  {
    ...
    summary: "地铁异常KING-03 档案的侦测入口，它将普通城市数据转化为龙王巢穴的坐标证据",
    report: [
    ...
    related: ["失踪专员", "血系结", "地铁", "震动数据", "楚子", "诺玛"],
    ...
    directive: "建议以任务报告形式呈现：数据输入、路线重合、入口推断、现场进入、尼伯龙根确认"   },
  {
    ...
    related: ["楚子", "夏弥", "任务后创", "身份否认", "情感锚点", "未闭合节"],
...
const oceanWaterSections: BronzeFireSection[] = [
    ...
    directive: "归档时优先记录声呐回波、水压刻度、漂浮物证和断续通讯；未确认完整王座信号前，不建立本体模型"   },
  {
    ...
    summary: "深海遗迹应作KING-04 的候选战场，而不是确定地点。它负责承载未知、失联和无法回收的压迫感",
    report: [
    ...
    directive: "建立多重锁定槽位，每个槽位仅保留假设名称、证据等级和禁止回收标记"   },
  {
    ...
    report: [
      ...
      "若按水权柄推演，双生分工可能体现为“深海沉/ 潮汐外泄”“记忆保/ 证据抹除”“容/ 召回”等方向，但这些只能作为诺玛假设，不进入身份判定"     ],
    related: ["双生空位", "主王座节", "副王座节", "王座共鸣", "未解", "诺玛假设"],
    risk: "双生节点未确认前，任何单次击杀、单件遗骸回收或单一地点封锁都不能作为王座终止证明",
    directive: "保留两个水下空王座槽位：一个标PRIME SIGNAL LOST，另一个标TWIN SIGNAL UNRESOLVED"   },
  {
    id: "watch",
    label: "苏醒预警",
    level: "WATCH",
    ...
    report: [
      "任何持续潮汐逆转、船只集体失联、深水声呐空洞、遗迹坐标漂移、未知龙文浮现，都应提升KING-04 预警",
    ...
    directive: "双生节点未确认前，不得宣KING-04 王座终止；所有水域异常维持长期监控"   }
];
...
const blackKingSections: BronzeFireSection[] = [
  {
    ...
    related: ["尼德霍格", "龙皇", "黑王血", "四大君主", "古龙", "终末王座"],
    ...
    directive: "维持 PRIME-00 最高封存。除校长、诺玛核心与校董会授权席位外，禁止调阅完整谱系链"   },
  {
    ...
    related: ["言灵·皇", "3E考试", "血统共", "白王血", "龙皇召唤", "异常免疫"],
    risk: "误判该言灵反应会直接影响学院招生、隔离、血统评级和高危个体处置",
    directive: "所有无响应样本上调SUSPICIOUS BLOODLINE；完成白王侧、黑王侧与未知谱系三向复核前，不得公开评级结论"   },
  {
    id: "bloodline",
    label: "黑王血",
    level: "BLOODLINE",
    ...
    directive: "所有高血统专员必须保留龙皇召唤应急记录；诺玛需维持全校范围内的言灵反应静默监控"   },
  {
    ...
    related: ["诸神黄昏", "世界", "屠龙战争", "龙皇复苏", "终末预案", "秘党"],
    risk: "一PRIME-00 进入 ACTIVE 状态，常规保密协议失效，学院必须转入文明级存续响应",
...
const whiteKingSections: BronzeFireSection[] = [
    ...
    directive: "PRIME-01 维持独立封存，不并入四大君主序列。涉及白王血裔、圣骸和神谕的记录必须同步校长室"   },
  {
    ...
    related: ["言灵·神", "言灵·皇", "白王血", "血统免", "叛乱协议", "路明非异"],
    ...
    directive: "建立 ORACLE WATCH。凡对皇帝无响应且出现高血统迹象者，必须进入白王侧复核流程"   },
  {
    ...
    related: ["白王血", "隔离预案", "血统评", "3E考试", "古德里安", "曼施坦因"],
    ...
    directive: "白王血裔疑似样本实行双签名制度：谱系学教授与执行部负责人共同确认后，方可上调隔离等级"   },
  {
    ...
    report: [
      ...
      "赫尔佐格事件被列PRIME-01 最高危案例：人类阴谋、白王遗产、龙类进化和现代武装处置在同一战场重叠，证明白王档案不能仅作为古代史处理"     ],
    ...
    directive: "所有圣骸相关物证执行零接触原则。未经校长室、装备部和诺玛三方确认，不得移动、切割或植入"   },
  {
    ...
    related: ["蛇岐八家", "猛鬼", "红井", "赫尔佐格", "源稚", "源稚", "上杉绘梨"],
    risk: "地区组织一旦掌握王座遗产，会把屠龙任务转化为内部权力工程，导致学院情报长期失真",
    directive: "日本线相关记录维CASE-JP 封存。任何白王遗产再发现，必须跳过地区分部直接回传诺玛核心"   }
];
...
const kuimenMissionSections: BronzeFireSection[] = [
  {
    ...
    label: "任务简",
    level: "OBJECTIVE",
    status: "长江水域 / 青铜城确/ 王座物证",
    fileNo: "ED-KUIMEN-BRIEF",
    classification: "执行部任/ S 级水下行",
    ...
    related: ["夔门计划", "摩尼亚赫", "长江水域", "青铜", "青铜与火之王"],
    risk: "任务目标从遗迹确认升级为王座宫殿接触后，S 级侦察标准不足以覆盖后续追击、伤亡和物证污染风险",
    ...
    directive: "战场图必须以动态剖面记录：水面、入口、宫殿、撤离通路、追击区五层并列"   },
  {
    ...
    directive: "时间轴节点不得删除。后续训练模拟必须保ROUTE LOST OXYGEN FAILURE 两个强制失败条件"   },
  {
    id: "casualty",
    label: "伤亡记录",
    level: "CASUALTY",
    ...
    directive: "所有回收物证执行封存柜协议：编号、取得者、取得地点、当前状态、异常反应五项必填"   },
  {
    ...
    related: ["王座级行", "二次作战", "执行部训", "水下撤离", "诺玛复盘"],
    risk: "若复盘结论未进入执行部训练体系，后续水下王座任务将重复同类伤亡",
    directive: "将夔门计划列S 级行动教材。所有水下行动预案必须加入活体权限、路径变更和氧气失效三项推演"   }
];
...
const beijingNibelungSections: BronzeFireSection[] = [
    ...
    directive: "所有地铁异常与失踪记录合并归档，禁止拆分为交通故障、地质波动和普通失踪案"   },
  {
    ...
    label: "指挥",
    level: "COMMAND",
    status: "楚子航独立调/ 诺玛数据复核",
    fileNo: "ED-BJ-NIBELUNG-COMMAND",
    classification: "指挥档案 / 非完整链",
    ...
    directive: "所有涉及夏弥的记录同步 KING-03 档案，禁止低于龙王级权限处理"   },
  {
    ...
    classification: "战场档案 / 城市覆盖",
    summary: "战场由三层空间构成：地表城市、地铁交通系统、尼伯龙根巢穴。三层坐标并不完全同步",
    report: [
      "地铁线路是入口外壳。站台、隧道、列车与卷闸门均可能成为现实接口，普通地图无法表示完整空间结构",
      "地下巢穴KING-03 双生节点的核心区域。芬里厄守巢、地动异常和隐藏站点共同构成王座空间"     ],
    related: ["地表城市", "北京地铁", "隐藏站台", "芬里厄巢", "地下空间"],
    risk: "错误坐标会导致救援队进入现实地铁而非尼伯龙根，或在尼伯龙根内失去回到现实的接口",
    directive: "建立三层坐标档案，所有坐标必须标REAL / METRO / NEST 三态"   },
  {
    id: "timeline",
    label: "时间",
    level: "TIMELINE",
    ...
    related: ["震动数据", "路线重合", "隐藏入口", "芬里", "夏弥身份暴露"],
    ...
    directive: "保留 IDENTITY EXPOSED 节点，作为后续训练模拟的强制转折"   },
  {
    ...
    related: ["失踪专员", "夏弥", "楚子", "任务后效", "精神残留"],
    risk: "若只记录死亡结果，会遗漏身份伪装对执行人员判断与心理状态的污染",
    directive: "伤亡档案必须KING-03 情感损耗条目互链"   },
  {
    id: "evidence",
    label: "物证回收",
    level: "EVIDENCE",
    status: "地铁/ 笔记/ 震动数据 / 影像残片",
    ...
    related: ["地铁", "异常震动数据", "失踪专员笔记", "尼伯龙根影像", "诺玛计算"],
    ...
    directive: "所有数据型物证执行双备份：诺玛核心、执行部离线封存各保留一份"   },
  {
    ...
    related: ["城市覆盖", "身份伪装", "双生王座", "KING-03", "执行部训"],
...
const beijingArchiveLayers: MissionArchiveLayer[] = [
  {
    ...
    points: [
      "地铁线路是现实接口，隐藏站台是过渡阈值，巢穴层才KING-03 的核心空间",
      "诺玛只能建立叠合模型，无法以普通城市地图复原完整路径",
      "入口条件与列车、站台、夜间震动和血统接触有关，普通人员可能在无感状态下越界"     ],
    directive: "所有坐标必须标REAL / METRO / NEST 三态，禁止使用单一地理坐标发布救援指令"   },
  {
    id: "king",
    code: "KING-03",
    title: "龙王接触档案",
    ...
    points: [
      "夏弥的人类身份不得归入普通协同行动人员，应独立连KING-03 档案",
      "芬里厄巢穴与地动异常直接关联，属于王座空间的物理核心",
      "双生节点不能合并研判，伪装节点与守巢节点的威胁方式完全不同"     ],
    directive: "所有涉及夏弥、芬里厄、隐藏站台和地下巢穴的记录同KING-03 权限"   },
  {
    id: "blackbox",
    code: "BLACK BOX",
    title: "现场黑匣",
  ...
  mission_japan_containment: {
    ...
    sections: [
        ...
        directive: "所有白王遗产记录跳过普通地区权限，直接同步诺玛核心与校长室"       },
      {
        ...
        related: ["学院本部", "日本分部", "源氏重工", "蛇岐八家", "猛鬼"],
        ...
        directive: "建立三重指令记录：本部命令、地区回应、现场执行结果必须分栏保存"       },
      {
        ...
        label: "行动人员",
        level: "FIELD TEAM",
        status: "本部专员 / 日本混血/ 高危血",
        fileNo: "ED-JP-CONTAIN-TEAM",
        classification: "人员档案 / 血统污",
        ...
        directive: "地区组织档案采用双线并行格式，禁止只记录胜负关系"       },
      {
        ...
        related: ["源稚", "源稚", "上杉绘梨", "白王血", "情感锚点"],
        risk: "高危血裔一旦被组织目标、个人情感或圣骸流程绑定，处置代价会迅速超过任务预案",
        directive: "所有高危血裔档案同PRIME-01，不允许降级为地区人物档案"       },
      {
        id: "redwell",
        label: "红井行动",
        level: "RED WELL",
        ...
        related: ["红井", "圣骸", "赫尔佐格", "白王复苏", "最终收"],
        risk: "红井一旦进入复苏阶段，现场处置目标会从夺回物证变为阻断王座重生",
        directive: "红井相关坐标、影像、血样和人员记录一律归PRIME-01 红色封存柜"       },
      {
        id: "sea",
        label: "海域作战",
        level: "SEA ZONE",
    ...
    layers: [
      { id: "legacy", code: "LEGACY", title: "白王遗产", mapFocus: "evidence", summary: "记录圣骸、血统、实验与白王复苏链的连接方式", points: ["圣骸是核心物证", "白王血裔是活体风险", "实验记录是复苏流程图"], directive: "白王遗产不得由地区分部单独处理" },
      { id: "faction", code: "FACTION", title: "地区组织", mapFocus: "command", summary: "记录蛇岐八家、猛鬼众和日本分部的权限冲突", points: ["组织身份会遮蔽任务身份", "地区命令可能与本部命令冲突", "情报来源必须独立标注"], directive: "所有地区情报进入诺玛前必须执行来源评级" },
      { id: "well", code: "RED WELL", title: "红井封存", mapFocus: "site", summary: "记录红井作为白王遗产复苏现场的设施、运输和最终处置风险", points: ["红井不是普通设施", "地下结构具备封存与孵化双重意义", "海域外溢会扩大灾害面"], directive: "红井坐标按禁区处理，禁止开放给普通执行部训练" },
      { id: "after", code: "AFTERMATH", title: "后效评估", mapFocus: "assessment", summary: "记录日本任务对学院地区制度和白王档案的长期影响", points: ["地区分部需重新审计", "白王档案需独立于黑王体系", "高危血裔需要长期监控"], directive: "将日本线并入 PRIME-01 常设预警" }
    ]
  },
  mission_greenland_ice: {
    title: "格陵兰冰海记",
    ...
    sections: [
      { id: "brief", label: "任务简", level: "OBJECTIVE", status: "冰海失联 / 历史事故", fileNo: "ED-GL-ICE-BRIEF", classification: "执行部任/ 历史封存", summary: "格陵兰冰海记录保存一次早期失控行动的残留信息，是执行部纪律和深海禁令的重要来源", report: ["任务进入极地冰海后遭遇超出预案的龙类异常，通讯、定位和回收链先后失效", "该记录因人员失联、装备残留和污染风险长期封存"], related: ["格陵", "冰海", "失联", "潜水", "历史事故"], risk: "极地环境会放大通讯失效和救援延迟，任何异常都可能成为不可逆事故", directive: "历史事故不得简化为背景资料，必须作为行动纪律来源保存" },
      ...
      { id: "command", label: "指挥", level: "COMMAND", status: "远程指挥 / 回收失败", fileNo: "ED-GL-ICE-COMMAND", classification: "指挥档案 / 极地链路", summary: "冰海任务暴露远程指挥在极端环境中的薄弱环节", report: ["现场平台与本部之间链路不稳定，定位回传无法持续", "回收窗口被天气、冰层和水下异常同时压缩"], related: ["远程指挥", "回收窗口", "极地通讯", "诺玛记录"], risk: "远程指挥若缺少现场冗余，会在失联后无法重建任务态势", directive: "极地任务必须配置独立黑匣和返航判据" },
      { id: "team", label: "行动人员", level: "FIELD TEAM", status: "失联人员 / 禁止公开", fileNo: "ED-GL-ICE-TEAM", classification: "人员档案 / 限制访问", summary: "人员记录只保留行动角色、失联节点和封存等级，不开放普通身份细节", report: ["失联人员不得进入普通伤亡表，需与装备残留和污染评估绑定", "幸存或残留信息均按历史封存权限处理"], related: ["失联人员", "执行", "潜水", "封存名单"], risk: "错误公开会影响学院内部纪律和历史事故审计", directive: "人员详情维持限制访问，只向执行部审计开放" },
      { id: "site", label: "战场坐标", level: "BATTLESPACE", status: "冰层 / 深海 / 残骸", fileNo: "ED-GL-ICE-SITE", classification: "战场档案 / 极地水域", summary: "战场由冰面平台、冰下水域、残骸区和未知异常源构成", report: ["冰层阻断常规救援，深海压力限制搜索时长", "残骸区可能保留装备、血样或龙类接触痕迹"], related: ["冰层", "深海", "残骸", "异常"], risk: "坐标漂移会导致回收队重复进入危险区", directive: "极地坐标ICE / SEA / WRECK 三层保存" },
      { id: "lastsignal", label: "最后信", level: "BLACK BOX", status: "通讯中断 / 定位漂移 / 回传残缺", fileNo: "ED-GL-ICE-LASTSIGNAL", classification: "黑匣档案 / 最后回", summary: "最后信号档案保存失联前的残缺通讯、定位漂移和生命体征异常，是判断事故性质的核心记录", report: ["黑匣记录显示通讯并非瞬间消失，而是经历噪声升高、坐标漂移、回传中断和生命体征失真", "最后回传不能还原完整战斗，但足以证明现场异常具有主动干扰性质"], related: ["最后信", "通讯噪声", "定位漂移", "生命体征", "黑匣"],
        risk: "残缺信号若被过度解读，会造成错误追责；若被忽略，则会丢失后续禁令依据",
        directive: "黑匣音频、坐标和生命体征三类记录必须并排展示，不得抽离单独引用" },
      { id: "evidence", label: "物证封存", level: "EVIDENCE", status: "潜水/ 黑匣 / 残留", fileNo: "ED-GL-ICE-EVIDENCE", classification: "物证档案 / 冰海残留", summary: "物证以潜水服、黑匣数据和水下残留为主，证明事故并非普通灾害", report: ["潜水服残留记录是事故复盘的核心入口", "黑匣数据不完整，但足以支撑执行部禁令"], related: ["潜水", "黑匣", "残留", "极地样本"], risk: "物证可能携带精神污染或龙类接触残留", directive: "冰海物证禁止教学展示，只可用于封闭复盘" },
      { id: "aftereffect", label: "后效记录", level: "AFTERMATH", status: "幸存者阴/ 纪律来源 / 长期封存", fileNo: "ED-GL-ICE-AFTERMATH", classification: "后效档案 / 心理与纪",
        ...
        related: ["幸存者档", "心理后效", "深海禁令", "行动审计"],
        risk: "若只将该事故作为悲剧叙事保存，后续执行部仍会重复同样的链路错误",
        directive: "后效档案与训练禁令绑定，所A 级以上水域任务必须调阅" },
    ...
    layers: [
      { id: "ice", code: "ICE", title: "极地环境", mapFocus: "site", summary: "记录冰面、冰下和深海环境对任务链路的压迫", points: ["冰层限制回收", "低温压缩行动窗口", "深海坐标难以稳定"], directive: "极地任务需设定硬撤离时间" },
      { id: "loss", code: "LOSS", title: "失联记录", mapFocus: "team", summary: "记录失联人员、信号终止和最后回传节点", points: ["人员记录与黑匣绑定", "失联不是普通伤亡", "身份细节限制公开"], directive: "失联记录不得被普通化处理" },
      ...
      { id: "ban", code: "BAN", title: "纪律禁令", mapFocus: "assessment", summary: "记录该事故转化为执行部任务纪律的过程", points: ["撤离窗口必须硬化", "远程链路必须冗余", "未知区域禁止孤线深入"], directive: "极地禁令作为强制训练条目" }
    ]
  },
  mission_cassell_invasion: {
    title: "卡塞尔学院入侵事",
    subtitle: "MISSION-S / 校内战情 / 中央控制室警",
    color: "#ff7e60",
    mapVariant: "cassell",
    mapTitle: "CASSELL / CAMPUS DEFENSE GRID",
    coordinate: "LIBRARY / CONTROL ROOM / ICE CELLAR",
    sections: [
      { id: "brief", label: "任务简", level: "OBJECTIVE", status: "校内入侵 / 战情启动", fileNo: "ED-CSL-INV-BRIEF", classification: "执行部任/ 校内战情", summary: "该事件记录卡塞尔学院内部遭遇武装或异常入侵时的防御流程", report: ["警报由校内安保系统触发，中央控制室进入战情模式", "任务目标从识别入侵者升级为保护核心设施、学生和封存物"], related: ["卡塞尔学", "中央控制", "冰窖", "学生", "狮心"], risk: "校内战场会使学生、教授、封存物和入侵目标混杂", directive: "校内战情必须优先隔离冰窖和中央控制室" },
      { id: "command", label: "指挥", level: "COMMAND", status: "诺玛 / 教授/ 学生组织", fileNo: "ED-CSL-INV-COMMAND", classification: "指挥档案 / 校内防御", summary: "指挥链由诺玛、教授团、执行部留守力量和学生组织共同组成", report: ["诺玛负责警报、门禁、监控和广播", "学生会与狮心会可作为临时防御力量，但需避免越权进入封存区"], related: ["诺玛", "教授", "学生", "狮心"], risk: "多组织同时响应会造成路线冲突和火力误判", directive: "校内战情启动后，所有行动组按区域权限接收路线" },
      { id: "control", label: "中央控制", level: "CONTROL",
        ...
        related: ["中央控制", "诺玛", "门禁系统", "校园广播", "教授"],
        risk: "若控制室被入侵者夺取，敌方可反向利用门禁和广播制造大规模混乱",
        directive: "中央控制室列COMMAND-CORE，校内警报启动后自动进入双人授权"       },
      { id: "team", label: "行动人员", level: "FIELD TEAM", status: "学生战斗/ 教授支援", fileNo: "ED-CSL-INV-TEAM", classification: "人员档案 / 校内动员", summary: "该任务的行动人员同时具备学生身份和战斗身份，需在档案中拆分记录", report: ["恺撒、楚子航等高阶学生可进入临时战斗链", "普通学生必须被导入避难路线，不得参与核心区防御"], related: ["恺撒", "楚子", "学生", "狮心", "教授"], risk: "学生战斗力可用，但心理、权限和纪律风险高", directive: "校内动员必须标注临时授权范围" },
      { id: "site", label: "战场坐标", level: "BATTLESPACE", status: "校园 / 图书/ 中央控制/ 冰窖", fileNo: "ED-CSL-INV-SITE", classification: "战场档案 / 校园防御", summary: "战场由校园开放区、教学建筑、图书馆、中央控制室和冰窖构成", report: ["图书馆与中央控制室是信息核心，冰窖是高危封存核心", "入侵路线必须与门禁日志、监控盲区和学生避难路线叠合"], related: ["图书", "中央控制", "冰窖", "门禁", "监控"], risk: "入侵者若进入冰窖，事件等级会立即上调", directive: "校园图必须标PUBLIC / COMMAND / SEALED 三层" },
      { id: "route", label: "入侵路线", level: "ROUTE",
        ...
        directive: "路线图必须同时显ENEMY / CIVILIAN / RESPONSE 三条线"       },
      { id: "icecellar", label: "冰窖封锁", level: "SEALED",
        status: "封存/ 高危入口 / 权限上锁",
        fileNo: "ED-CSL-INV-ICECELLAR",
        classification: "设施档案 / 高危封存",
    ...
    layers: [
      { id: "campus", code: "CAMPUS", title: "校园防御", mapFocus: "site", summary: "记录校园开放区、图书馆、控制室和冰窖的分层防御", points: ["开放区负责疏散", "控制室负责指挥", "冰窖必须隔离"], directive: "校内战场按权限层封锁" },
      { id: "alarm", code: "ALARM", title: "诺玛警报", mapFocus: "command", summary: "记录警报、广播、门禁和监控系统的联动", points: ["警报需自动分级", "门禁日志保留原始时间戳", "广播不得暴露封存区"], directive: "警报链路进入中央控制室复盘" },
      { id: "student", code: "TEAM", title: "学生动员", mapFocus: "team", summary: "记录学生组织进入战情链的权限和风险", points: ["高阶学生可临时授权", "普通学生进入避难路线", "战斗身份与学生身份分开记录"], directive: "临时授权不得延伸至封存区" },
      { id: "sealed", code: "SEALED", title: "冰窖隔离", mapFocus: "evidence", summary: "记录冰窖、封存物和入侵路线之间的防护关系", points: ["冰窖是最高危节点", "封存物不得移动", "入侵接近即升级"], directive: "冰窖封锁优先级高于普通追击" }
    ]
  },
  mission_bronze_second: {
    title: "青铜计划 / 二次作战",
    subtitle: "MISSION-SS / 摩尼亚赫/ 风暴鱼雷处置",
    color: "#e2bd64",
    mapVariant: "bronzeSecond",
    mapTitle: "BRONZE PLAN / SECOND STRIKE",
    coordinate: "RIVER SURFACE / TARGET LURE / TORPEDO WINDOW",
    sections: [
      { id: "brief", label: "任务简", level: "OBJECTIVE", status: "二次进入 / 龙王处决", fileNo: "ED-BRZ-SECOND-BRIEF", classification: "执行部任/ SS 级处决行", summary: "青铜计划是夔门计划之后的二次作战，目标从确认青铜城升级为诱导并处决青铜与火之王", report: ["行动以摩尼亚赫号为水面平台，利用诱导、定位和重武器完成单发窗口处置", "任务必须在龙王完全恢复前完成，否则水面平台无法承受反击"], related: ["青铜计划", "摩尼亚赫", "风暴鱼雷", "诺顿", "七宗"], risk: "二次作战成败取决于单一窗口，任何延迟都会使处决失败", directive: "青铜计划记录必须与夔门计划和 KING-01 档案互链" },
      { id: "kuimenAfter", label: "夔门后续", level: "FOLLOW-UP",
        ...
        related: ["夔门计划", "黄铜", "金属", "王座复苏", "二次授权"],
        risk: "若二次作战延迟，诺顿可能脱离脆弱复苏阶段，进入常规武器无法处理的完全状态",
        directive: "青铜计划所有条目必须回链夔门计划，不得作为孤立处决行动展示"       },
      { id: "command", label: "指挥", level: "COMMAND", status: "摩尼亚赫/ 装备/ 学生行动", fileNo: "ED-BRZ-SECOND-COMMAND", classification: "指挥档案 / 水面处决", summary: "指挥链围绕摩尼亚赫号、装备部武器、学生行动组和诺玛火控记录展开", report: ["现场指挥需同时处理船体损伤、目标诱导、鱼雷授权和人员撤离", "装备部方案具备高破坏力，也具备高不确定性"], related: ["曼施坦因", "摩尼亚赫", "装备", "诺玛火控"], risk: "指挥延迟会导致鱼雷窗口错失或误击友方", directive: "所有火控命令保留双确认记录" },
      { id: "team", label: "行动人员", level: "FIELD TEAM", status: "恺撒 / / 诺诺 / 路明", fileNo: "ED-BRZ-SECOND-TEAM", classification: "人员档案 / 二次行动", summary: "行动人员承担诱导、接触、火控协助和应急判断，风险高于普通学生任务", report: ["恺撒、零、诺诺、路明非等节点分别进入侦测、执行和异常触发链", "路明非相关记录需单独标注，因为其异常权限可能影响最终处置窗口"], related: ["恺撒", "", "诺诺", "路明", "学生行动"], risk: "学生行动组靠近王座目标时，战斗风险与权限风险同步升高", directive: "行动人员记录必须连接身份档案与青铜与火之王档案" },
      { id: "lure", label: "诱导计划", level: "LURE",
        ...
        related: ["目标诱导", "火控窗口", "撤离", "摩尼亚赫", "诺顿"],
        risk: "诱导距离过近会导致船体和人员被同时卷入处决范围；距离过远则鱼雷窗口失效",
        directive: "诱导计划必须TORPEDO WINDOW 同步显示，不允许单独批准"       },
      { id: "site", label: "战场坐标", level: "BATTLESPACE", status: "水面平台 / 龙王目标 / 鱼雷航道", fileNo: "ED-BRZ-SECOND-SITE", classification: "战场档案 / 水面打击", summary: "战场由摩尼亚赫号、龙王目标、鱼雷航道、风暴区和撤离线构成", report: ["目标需被诱导至鱼雷可命中区域", "风暴、水流和船体损伤都会改变打击窗口"], related: ["水面平台", "鱼雷航道", "风暴", "目标诱导"], risk: "战场条件不稳定会造成鱼雷偏离或船体失控", directive: "战场图必须显TARGET / TORPEDO / EVAC 三线" },
      { id: "torpedo", label: "风暴鱼雷", level: "TORPEDO",
        ...
        related: ["风暴鱼雷", "装备", "火控授权", "单发窗口", "船体姿"],
        risk: "鱼雷若偏离目标，既可能误伤友方，也可能促使龙王脱离脆弱阶段",
        directive: "风暴鱼雷记录需保存发射前后完整火控数据，进入装备部与执行部双封存"       },
      { id: "evidence", label: "物证封存", level: "EVIDENCE", status: "七宗/ 龙王残留 / 火控记录", fileNo: "ED-BRZ-SECOND-EVIDENCE", classification: "物证档案 / 处决证据", summary: "物证用于证明青铜与火之王处决窗口、武器效果和后续封存状态", report: ["七宗罪与鱼雷处置记录共同构成王座级武器链", "龙王残留、火控数据和船体损伤记录必须同步保存"], related: ["七宗", "火控记录", "龙王残留", "船体损伤"], risk: "处决证据若不完整，会影响后续判断王座是否真正终止", directive: "处决证据进入 KING-01 封存柜" },
      { id: "sevensins", label: "七宗罪链", level: "ARSENAL",
      ...
      { id: "lumingfei", label: "异常节点",
        ...
        related: ["路明", "S 级权", "异常节点", "结果扰动", "身份档案"],
    ...
    layers: [
      { id: "lure", code: "LURE", title: "目标诱导", mapFocus: "team", summary: "记录如何迫使龙王进入可处决窗口", points: ["诱导目标靠近水面平台", "人员暴露度必须受控", "异常权限需单独记录"], directive: "诱导行动不得脱离撤离线" },
      { id: "torpedo", code: "TORPEDO", title: "风暴鱼雷", mapFocus: "site", summary: "记录鱼雷航道、火控窗口和命中判定", points: ["鱼雷窗口极短", "火控命令需双确认", "水流与风暴会改变航道"], directive: "鱼雷发射前必须锁定友方位置" },
      { id: "weapon", code: "ARSENAL", title: "炼金武器", mapFocus: "evidence", summary: "记录七宗罪、火控数据和龙王处决证据", points: ["七宗罪连KING-01", "处决证据必须完整", "船体损伤同步封存"], directive: "炼金武器记录同步炼金资料库" },
      { id: "review", code: "REVIEW", title: "二次作战复盘", mapFocus: "assessment", summary: "记录青铜计划作为王座级二次作战模板的复盘结论", points: ["单发窗口不可过度依赖", "失败预案必须前置", "王座终止需证据闭环"], directive: "所有王座处决任务必须包含备份方案" }
    ]
  }
};
  ...
  return (
    <section className="bronze-fire-archive" style={{ "--holo-color": "#e2bd64" } as CSSProperties}>
      ...
      <aside className="bronze-fire-command">
        ...
        <h1>青铜与火之王</h1>
        <p>
          KING-01 档案已解封。该王座关联诺顿、康斯坦丁、青铜城、三峡水下行动与七宗罪。青铜与火之王不是单纯的元素代号，而是一套完整的君王权柄：铸造、炼金、火焰、金属、宫殿、武器，以及双生之间无法切断的召唤"        </p>
  ...
  return (
    <section className="bronze-fire-archive sky-wind-archive" style={{ "--holo-color": "#8ee7ff" } as CSSProperties}>
      ...
      <aside className="bronze-fire-command">
        ...
        <h1>天空与风之王</h1>
        <p>
          KING-02 档案聚焦天空与风之王王座本身：风权柄、风系样本、言灵参照、苏醒监测与处置原则。北京尼伯龙根中的镰鼬群仅作为风系灾害案例收录，不作为王座本体判定依据"        </p>
        <div className="bronze-fire-section-list">
...
function EarthMountainArchive({ onClose }: { onClose: () => void }) {
  return (
    <DragonKingDossierArchive
      ...
      intro="KING-03 档案已进入确认级别：北京地铁异常、尼伯龙根入口、夏弥身份、芬里厄巢穴与双生王座共同构成大地与山之王的完整风险面"       color="#c7b06a"
...
function BlackKingArchive({ onClose }: { onClose: () => void }) {
  return (
    <DragonKingDossierArchive
      ...
      intro="PRIME-00 档案已进入最高封存：尼德霍格、言灵·皇帝、黑王血裔、终末预案与龙族谱系源头均归入龙皇王座"       color="#d8c27a"
...
function WhiteKingArchive({ onClose }: { onClose: () => void }) {
  return (
    <DragonKingDossierArchive
      ...
      intro="PRIME-01 档案维持禁忌封存：白王叛乱、言灵·神谕、白王血裔、圣骸与日本收容链均归入独立王座风险"       color="#f0e8c8"
...
function OceanWaterArchive({ onClose }: { onClose: () => void }) {
  return (
    <DragonKingDossierArchive
      ...
      intro="KING-04 档案保持未解封状态。当前只记录水域权柄、深海遗迹、沉眠容器、双生空位与苏醒预警；姓名、地点和主副节点均等待后续证据确认"       color="#75d8ff"
...
function KuimenMissionMap({ activeId }: { activeId: string }) {
  ...
  const points = [
    { id: "brief", label: "BRIEF / 简", x: 9, y: 23 },
    { id: "command", label: "COMMAND / 指挥", x: 25, y: 31 },
    { id: "team", label: "DIVE TEAM / 下潜", x: 26, y: 52 },
    { id: "site", label: "BRONZE CITY / 青铜", x: 53, y: 61 },
  ...
  return (
    <div className="kuimen-map" aria-hidden="true">
      ...
      <div className={`kuimen-city-core${["site", "timeline", "evidence"].includes(activeId) ? " is-active" : ""}`}>
        <span>BRONZE CITY CORE / 青铜城核</span>
      </div>
...
function BeijingNibelungMap({ activeId }: { activeId: string }) {
  ...
  const focusCards: Record<string, { title: string; rows: Array<[string, string]> }> = {
    brief: { title: "MISSION SNAPSHOT / 任务快照", rows: [["等级", "MISSION-SS"], ["目标", "地铁异常"], ["状", "数据建模"]] },
    command: { title: "COMMAND STATUS / 指挥状", rows: [["主节", "楚子"], ["支援", "诺玛"], ["链路", "不完"]] },
    team: { title: "FIELD CHAIN / 行动", rows: [["楚子", "现场调查"], ["路明", "接触"], ["夏弥", "KING-03 锁定"]] },
    site: { title: "BATTLESPACE / 战场坐标", rows: [["现实", "北京"], ["交通层", "地铁"], ["巢穴", "尼伯龙根"]] },
    timeline: { title: "TIMELINE / 时间", rows: telemetry.comm.map((line) => { const [time, status] = line.split(" / "); return [time, status ?? line]; }) },
    casualty: { title: "CASUALTY / 伤亡后效", rows: [["失踪专员", "MIA / 关联确认"], ["夏弥", "KIA / 龙王处置"], ["楚子", "RESIDUAL / 残留"]] },
    evidence: { title: "EVIDENCE LOCKER / 物证封存", rows: [["地铁", "SEALED"], ["笔记", "RESTRICTED"], ["震动数据", "BACKUP"], ["影像残片", "DAMAGED"]] },
    assessment: { title: "NORMA ASSESS / 诺玛复盘", rows: [["案例等级", "MISSION-SS"], ["关联王座", "KING-03"], ["训练模板", "REQUIRED"]] }
  };
  const focusCard = focusCards[activeId] ?? focusCards.brief;
  const nodes = [
    { id: "brief", label: "BRIEF / 简", x: 17, y: 28 },
  ...
  return (
    <div className="beijing-map" aria-hidden="true">
      ...
      <div className="beijing-station station-real">REAL CITY / 现实</div>
  ...
  return (
    <section className="mission-ops-archive beijing-ops-archive" style={{ "--holo-color": "#b44c3f" } as CSSProperties}>
      <aside className="mission-phase-list">
        <span>EXECUTIVE DEPARTMENT / DEEP OPERATION</span>
        <h1>北京尼伯龙根事件</h1>
        <p>MISSION-SS / 城市级异/ KING-03 接触</p>
        <div>
...
function GenericMissionPlanMap({ dossier, activeId }: { dossier: MissionDeepDossier; activeId: string }) {
  const focusCards: Record<string, { title: string; rows: Array<[string, string]> }> = {
    brief: { title: "MISSION SNAPSHOT / 任务快照", rows: [["等级", dossier.subtitle.split(" / ")[0]], ["目标", dossier.title], ["状", "建模"]] },
    command: { title: "COMMAND / 指挥", rows: [["链路", "多节"], ["授权", "诺玛记录"], ["状", "待复"]] },
    team: { title: "FIELD TEAM / 行动人员", rows: [["人员", "高危"], ["权限", "临时"], ["状", "接触"]] },
    site: { title: "BATTLESPACE / 战场坐标", rows: [["坐标", "多层"], ["区域", "限制"], ["出口", "待确"]] },
    timeline: { title: "TIMELINE / 时间", rows: [["阶段", "推进"], ["节点", "封存"], ["复盘", "记录"]] },
    casualty: { title: "CASUALTY / 伤亡记录", rows: [["等级", "限制"], ["后效", "评估"], ["归档", "封存"]] },
    evidence: { title: "EVIDENCE / 物证封存", rows: [["状", "SEALED"], ["备份", "NORMA"], ["权限", "限制"]] },
    assessment: { title: "ASSESS / 诺玛复盘", rows: [["结论", "生成"], ["模板", "可训"], ["风险", "持续"]] }
  };
  const focusCard = focusCards[activeId] ?? focusCards.brief;
  const variantNodes: Record<MissionDeepDossier["mapVariant"], Array<{ id: string; label: string; x: number; y: number }>> = {
    japan: [
      ...
      { id: "team", label: "FIELD CELL / 行动", x: 30, y: 58 },
    ...
    greenland: [
      ...
      { id: "team", label: "DIVE SUIT / 潜水", x: 42, y: 52 },
      { id: "site", label: "WRECK ZONE / 残骸", x: 58, y: 68 },
      { id: "timeline", label: "SIGNAL LOST / 信号终止", x: 70, y: 42 },
      { id: "casualty", label: "MIA / 失联", x: 82, y: 62 },
      { id: "evidence", label: "LOCKER / 潜水", x: 68, y: 80 },
      { id: "assessment", label: "BAN / 禁令", x: 88, y: 34 }
    ],
    cassell: [
      { id: "brief", label: "GATE / 校门", x: 16, y: 52 },
      { id: "command", label: "CONTROL / 中央控制", x: 48, y: 30 },
      { id: "team", label: "STUDENT TEAM / 学生战斗", x: 34, y: 68 },
      { id: "site", label: "LIBRARY / 图书", x: 54, y: 54 },
    ...
    bronzeSecond: [
      ...
      { id: "command", label: "MONIACH / 摩尼亚赫", x: 30, y: 44 },
      { id: "team", label: "LURE TEAM / 诱导", x: 44, y: 34 },
      ...
      { id: "evidence", label: "ARSENAL / 七宗", x: 61, y: 76 },
...
function SurveillanceGlobePanel({
  ...
  const selectedReviewLog =
    selectedReviewStatus === "EXECUTIVE REVIEW"
      ? "执行部旁路监听已加入该异常。NORMA 将等待二次证据交叉验证"       : selectedReviewStatus === "ARCHIVE LINKED"
        ? "证据库与相关龙王档案索引已建立。受限内容仍遵循当前专员权限"         : selectedReviewStatus === "WATCH"
          ? "异常已转入持续观察。系统不会生成强制任务，仅保留态势记录"           : "等待专员复核。该异常不会被视为主线任务";
  ...
  const reviewActions: Array<{ id: AnomalyReviewState; label: string; detail: string }> = [
    { id: "EXECUTIVE REVIEW", label: "请求执行部复", detail: "把异常交给执行部旁路监听，但不生成强制行动" },
  ...
  const selectedForecastLog =
    selectedForecastStatus === "ELEVATE FORECAST"
      ? "该灾害信号已被上调为龙王复苏预测，NORMA 将扩大采样范围并等待第二组自然灾害数据交叉验证"       : selectedForecastStatus === "LINK ARCHIVE"
        ? "相关龙王档案与灾害信号已建立索引。受限内容仍遵循当前专员权限"         : selectedForecastStatus === "KEEP WATCH"
          ? "该信号保留为持续监听状态。系统不会生成强制任务，仅更新复苏概率曲线"           : "等待专员判读。该记录代表自然灾害征兆，不代表事件已经发生";
      ...
      <div className="surveillance-globe-layout">
        <div className="surveillance-earth-wrap" aria-label="可拖动旋转的地球投影">
          ...
          <div className="surveillance-link-status">
            ...
            <strong>{forecastSourceLabel} / {forecastUpdatedAt ? new Date(forecastUpdatedAt).toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" }) : "同步"}</strong>
          </div>
        </div>
        <div className="surveillance-anomaly-list">
          ...
          <article className="surveillance-daily-brief">
            ...
            <p>
              {leadingKingForecast
                ? `主风险指${leadingKingForecast.king}。NORMA 已关${forecastSignals.length} 条自然灾害征兆，其中 ${readableSignalCount} 条可读，${restrictedSignalCount} 条需更高权限复核。`
          ...
          {activeProjection ? (
            <article className="surveillance-projection-brief">
              <div>
                ...
                <p>{formatEvidenceCount(activeProjection.signals.length)} / 最高概{activeProjection.probability}% / {formatForecastTrend(activeProjection.trend)}</p>
              </div>
              <button
                type="button"
                onClick={() => {
...
function FieldEvidenceLibraryPanel({
  module,
  profile,
  onAccessLog
}: {
  ...
  useEffect(() => {
    setForecastSignals(loadCachedForecastSignals());
  }, []);

  const evidenceList = useMemo(() => {
    const realtime = forecastSignals.map(mapForecastSignalToEvidence);
    return [...realtime, ...operationFieldEvidence].sort((a, b) => {
      const aLocked = clearance < a.clearance ? 1 : 0;
      const bLocked = clearance < b.clearance ? 1 : 0;
      if (aLocked !== bLocked) return aLocked - bLocked;
      return b.probability - a.probability;
    });
  }, [clearance, forecastSignals]);

  const filteredEvidence = evidenceList.filter((item) => {
    if (filter === "realtime") return item.kind === "realtime";
    if (filter === "operation") return item.kind === "operation";
    if (filter === "restricted") return clearance < item.clearance;
    return true;
  });

  const selectedEvidence =
    filteredEvidence.find((item) => getFieldEvidenceKey(item) === selectedEvidenceKey) ?? filteredEvidence[0] ?? null;
  const selectedLocked = selectedEvidence ? clearance < selectedEvidence.clearance : false;
  const readableCount = evidenceList.filter((item) => clearance >= item.clearance).length;
  const restrictedCount = evidenceList.length - readableCount;
  const sourceStatus = getEvidenceSourceStatus(evidenceList);
  const evidenceChains = useMemo(() => {
    return dragonKingForecastOrder
      .map((king) => {
        const items = filteredEvidence
          .filter((item) => item.king === king)
          .sort((a, b) => b.probability - a.probability);
        const maxProbability = items[0]?.probability ?? 0;
        const requiredClearance = Math.max(1, ...items.map((item) => item.clearance));
        const lockedCount = items.filter((item) => clearance < item.clearance).length;
        const signals = Array.from(new Set(items.flatMap((item) => item.signals))).slice(0, 5);
  ...
  }, [clearance, filteredEvidence]);

  const selectEvidence = (item: FieldEvidence) => {
    setSelectedEvidenceKey(getFieldEvidenceKey(item));
    const locked = clearance < item.clearance;
    onAccessLog?.({
  ...
  return (
    <section
      className="field-evidence-library"
      style={{ "--holo-color": module.color } as CSSProperties}
      aria-label="现场证据库"
    >
      <header className="field-evidence-header">
        <div>
          <span>TEXT EVIDENCE LIBRARY / FIELD SIGNAL INDEX</span>
          <h1>现场证据库</h1>
      ...
      <div className="field-evidence-summary">
        <article>
          <span>证据接入</span>
          <strong>{evidenceList.filter((item) => item.kind === "realtime").length}</strong>
          <em>实时灾害源 / NORMA 归类</em>
        </article>
        <article>
          <span>执行部证据</span>
          <strong>{operationFieldEvidence.length}</strong>
          <em>任务复盘 / 现场读数</em>
        </article>
        <article>
          <span>当前可读</span>
      ...
      <div className="field-evidence-source-status" aria-label="数据源状态">
        {sourceStatus.map((status) => (
          <span key={status.source}>
            {getEvidenceSourceLabel(status.source)} / {status.count} 条
          </span>
        ))}
      </div>

      <div className="field-evidence-filters" aria-label="证据筛选">
        {(["all", "realtime", "operation", "restricted"] as FieldEvidenceFilter[]).map((item) => (
          <button
            key={item}
            type="button"
          ...
          >
            {getEvidenceFilterLabel(item)}
          </button>
        ))}
      </div>

      <div className="field-evidence-view-tabs" aria-label="证据库视图">
        <button type="button" className={view === "chain" ? "is-active" : ""} onClick={() => setView("chain")}>
          证据链
        </button>
      ...
      {view === "chain" ? (
        <div className="field-evidence-chain-board" aria-label="龙王证据链">
          {evidenceChains.map((chain) => (
            <article key={chain.king} className={`field-evidence-chain${chain.lockedCount ? " has-locked" : ""}`}>
              <header>
                <div>
                  <span>{dragonKingSignalProfiles[chain.king]?.short ?? "王座"}</span>
                  <h2>{chain.king}</h2>
                </div>
                <strong>
              ...
              <div className="field-evidence-chain-line" aria-hidden="true">
                {chain.items.slice(0, 5).map((item, index) => (
                  <i key={getFieldEvidenceKey(item)} style={{ left: `${10 + index * 20}%` }} />
                ))}
              </div>
              <div className="field-evidence-chain-items">
                {chain.items.slice(0, 5).map((item) => {
                  const locked = clearance < item.clearance;

                  return (
                    <button
                      key={getFieldEvidenceKey(item)}
                      type="button"
              ...
              <div className="field-evidence-chain-signals">
                {chain.signals.map((signal) => (
        ...
        <div className="field-evidence-layout">
          <div className="field-evidence-list" aria-label="证据列表">
            {filteredEvidence.map((item) => {
              const locked = clearance < item.clearance;
              const active = selectedEvidence ? getFieldEvidenceKey(selectedEvidence) === getFieldEvidenceKey(item) : false;
              const status = getEvidenceStatus(item, clearance);

              return (
                <button
                  key={getFieldEvidenceKey(item)}
                  type="button"
                  className={`field-evidence-card${active ? " is-active" : ""}${locked ? " is-locked" : ""}`}
                ...
                >
                  ...
                  <h2>{item.title}</h2>
                  <p>
                    {getEvidenceSourceLabel(item.source)} / {item.category} / {item.king}
                  </p>
                  <div className="field-evidence-card-meta">
                    <em>{item.kind === "realtime" ? "实时接入" : "执行部"}</em>
                    <strong>{item.probability}%</strong>
                  </div>
                </button>
              );
          ...
          <aside className={`field-evidence-detail${selectedLocked ? " is-locked" : ""}`} aria-label="证据详情">
            {selectedEvidence ? (
              <>
                <header>
                ...
                <div className="field-evidence-detail-grid">
                  <div>
                    <span>来源</span>
                    <strong>{getEvidenceSourceLabel(selectedEvidence.source)}</strong>
                  </div>
                  <div>
                    <span>归属</span>
                    <strong>{selectedEvidence.king}</strong>
                  </div>
                  <div>
                    <span>时间</span>
                    <strong>{formatForecastTime(selectedEvidence.occurredAt)}</strong>
                  </div>
                  <div>
                    <span>地点</span>
                    <strong>{selectedEvidence.locationName}</strong>
                  </div>
                  <div className="is-wide">
                    <span>坐标</span>
                    <strong>{selectedEvidence.coordinate}</strong>
                  </div>
                </div>
                ...
                {selectedLocked ? (
                  ...
                  <>
                    <p className="field-evidence-detail-summary">{selectedEvidence.summary}</p>
                    <div className="field-evidence-signal-list">
                      {selectedEvidence.signals.map((signal) => (
                        <em key={signal}>{signal}</em>
                      ))}
                    </div>
                    <div className="field-evidence-norma">
                      <span>NORMA 判断</span>
                      <p>{selectedEvidence.normaJudgement}</p>
                    </div>
                  </>
                )}
              </>
...
function ArchivePanel({
  ...
  if (module.id === "evidence") {
    return <FieldEvidenceLibraryPanel module={module} profile={profile} onAccessLog={onAccessLog} />;
  }
...
export default function HoloTerminal3D({
  ...
  const isFinger = agentName.trim() === "芬格";
